...
def extrage_cheltuieli_buget(df):
    ...
    df_filtrat = df.iloc[3:stop_row, [1, 10]] if pd.notna(stop_row) else df.iloc[3:, [1, 10]]  # Ajustează indexul coloanei la 10 pentru a corespunde coloanei 11 în Excel
...
if uploaded_file is not None:
    try:
        ...
        if not df_rezultate.empty:
            ...
            cheltuieli_buget = '\n'.join([f"{row[1]} - {row[10]} buc." for _, row in df_rezultate.iterrows()])
