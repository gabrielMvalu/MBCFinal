...
def transforma_date_tabel2(df):
    ...
    df_filtrat = df[~df.iloc[:, 1].isin(valori_de_eliminat) & df.index < stop_in[0]]
    ...
    for i, row in enumerate(df_filtrat.itertuples(), 1):
        ...
        um.append("buc")
        cantitate.append(row[4])  # Presupunem că 'Cantitate' este în coloana 4
        pret_unitar.append(row[3])  # Presupunem că 'Preț unitar (fără TVA)' este în coloana 3
        valoare_totala.append(row[5])  # Presupunem că 'Valoare Totală (fără TVA)' este în coloana 5
        nr_crt_counter += 1
